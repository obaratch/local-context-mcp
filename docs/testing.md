...
## 基本方針
- 仕様変更が絡む実装では、原則として `docs/` を先に更新し、その後に単体テスト、最後に実装の順で進める。
- 最初の起点になる単体テストでは、expected red を明示的に確認してから実装に入る。
- `describe` にはテスト種別と対象機能や対象ツールを書く。
- `describe` は対象モジュール名だけでなく、責務や観点ごとに分けてよい。
- `test` には入力条件と期待する振る舞いが読める名前を書く。
- `test` の文言は「〜すること」で統一する。
- 1つのテストでは、原則として1つの振る舞いだけを検証する。
- 単体テストの起点には、正常系だけでなく境界条件または異常系を少なくとも 1 つ含める。
- 正常系を先に書き、その後に境界値や異常系を書く。
- fallback を持つ機能では、順序そのものが読めるテスト名にする。
- 外部レスポンスを使うときは、匿名化した fixture を先に固定してからテストを書く。
- pure function に切り出せる部分がある場合は、`src/utils` などの下位レイヤの単体テストを先に固定してから上位の結合へ進む。
...
- 入力条件がある場合は、`test("TTL 内のキャッシュがある場合は GeoIP を呼ばずに返すこと", ...)` のように条件を先に書く。
- fallback を持つ機能では、`test("先頭 provider が失敗した場合は次の provider へフォールバックすること", ...)` のように順序が読める名前を優先する。
...
- 実装初期に import error を避けたい場合は、外殻だけ作って `NotImplementedError` を投げる形で red を作ってよい。
...
- Docker イメージ既定の `LOCAL_CONTEXT_STORE_DIR=/data` を使う前提で検証する。
